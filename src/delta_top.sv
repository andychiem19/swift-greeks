// -------------------------------------------------------------------------------------//
// delta_top : computes Black-Scholes call delta = N(d1) from the five parameters.      //
//                                                                                      //
...
module delta_top #(
    ...
    output logic done,
    output logic signed [WIDTH-1:0] delta   // N(d1), Q16.16
);

    localparam signed [WIDTH-1:0] QUARTER =  32'h00004000; //  0.25
    localparam signed [WIDTH-1:0] HALF    =  32'h00008000; //  0.5
    localparam signed [WIDTH-1:0] TWO     =  32'h00020000; //  2.0
    localparam signed [WIDTH-1:0] FOUR    =  32'h00040000; //  4.0
    localparam signed [WIDTH-1:0] NEG_FOUR=  32'hFFFC0000; // -4.0
...
// ----------------------------------------------------------------------------------//
    ...
    // shared hyperbolic vectoring core (ln then sqrt)
    ...
    always_ff @(posedge clk or negedge nrst) begin
        if (!nrst) begin
            ...
            case (state)
                ...
                // ln(S/K): hyperbolic vectoring x=S+K, y=S-K -> z_out = ln(S/K)/2
                LN_LOAD: begin
                ...
                // sqrt(T): hyperbolic vectoring x=T+0.25, y=T-0.25 -> x_out = sqrt(T)
                SQRT_LOAD: begin
                ...
                // num = ln(S/K) + (r + sigma^2/2)*T ; den = sigma*sqrt(T)
                COMPUTE: begin
                ...
                DIV_CHECK: begin
                    ...
                    if (den == 0) begin
                        ...
                        // ratio in [2,4): halve num so CORDIC converges, double result later
                        div_num   <= num >>> 1;
                        div_scale <= 1;
                        state     <= DIV_LOAD;
                    end else begin
                ...
                // division via linear vectoring: x=den, y=num, z=0 -> z_out = num/den
                DIV_LOAD: begin
                    lin_x_in <= den;
                    lin_y_in <= div_num;
                    lin_z_in <= '0;
